...
import { normalizeBundleRequest, HttpError } from "../src/validate";
import { loadSkillMarkdown } from "../src/skill";
import { listPodsAllNamespaces, listPodsNamespaced } from "../src/k8s-compat";
...
// ---------------------------------------------------------------------------
// hasString — same logic as in index.ts (not exported, so we replicate)
// ---------------------------------------------------------------------------
...
// Build a Fastify app that mirrors the routes in index.ts
function buildApp(opts?: {
  ...
  app.post("/v1/bundles", async (req, reply) => {
    try {
      const normalized = normalizeBundleRequest(req.body as any, config);
